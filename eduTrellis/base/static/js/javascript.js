// Banner javascript 
    window.addEventListener('DOMContentLoaded', function() {
      ...
      function nextSlide() { showSlide((bannerIndex + 1) % bannerSlides.length); }
      function prevSlide() { showSlide((bannerIndex - 1 + bannerSlides.length) % bannerSlides.length); }
      function resetTimer() {
        if(bannerTimer) clearInterval(bannerTimer);
        bannerTimer = setInterval(nextSlide, 2200);
      }
      bannerPrev.onclick = prevSlide;
...
window.addEventListener('DOMContentLoaded', function() {
  const carouselIds = ['video-carousel', 'live-carousel', 'ebook-carousel', 'test-carousel'];
